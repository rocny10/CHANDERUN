...
class Juego:
        ...
        try:
            ...
            with open('data/save.json','w') as f:
                json.dump({'refrescos':self.refrescos_sys.refrescos,
                           'totales':self.refrescos_sys.refrescos_totales}, f)
        except: pass
    ...
    def actualizar(self):
        ...
        if self.contadores['r'] > random.randint(180,300):
            nuevo = Refresco(ANCHO, SUELO_Y-80)
            valido = True
        ...
        if self.contadores['s'] > random.randint(120,200):
            self.sierras.append(Sierra(ANCHO))
            self.contadores['s'] = 0
        # Generar cajas
        self.contadores['c'] += 1
        if self.contadores['c'] > random.randint(150,250):
            self.cajas.append(Caja(ANCHO))
            self.contadores['c'] = 0
            
        # Mover y limpiar
        for l in (self.refrescos_lista, self.sierras, self.cajas):
            for obj in l[:]:
    ...
    def dibujar(self):
        ...
        if self.estado == "JUGANDO":
            for r in self.refrescos_lista: r.dibujar(self.pantalla)
            for s in self.sierras: s.dibujar(self.pantalla)
            for c in self.cajas: c.dibujar(self.pantalla)
